...
public class MyHashMap<K, V> implements Map61B<K, V> {
    ...
    /** Returns index (bucket) in the table corresponding to the given key. */
    ...
    private void resize(int capacity) {
        ...
        for (K key : keys) {
            int bucket = getBucket(key, temp.length);
            if (temp[bucket] == null) {
                temp[bucket] = createBucket();
            }
            temp[bucket].add(getNode(key));
        }
        buckets = temp;
    }
